...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/admin/islanders/${id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json", "X-Admin-Key": ADMIN_KEY }, body: JSON.stringify(body),
    })
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  const { id } = await params
  try {
    const res = await fetch(`${API_BASE}/api/admin/islanders/${id}`, { method: "DELETE", headers: { "X-Admin-Key": ADMIN_KEY } })
    const data = await res.json().catch(() => ({}))
    if (!res.ok) return proxyError(data, res.status)
    return NextResponse.json(data)
  } catch { return NextResponse.json({ message: "Service unavailable." }, { status: 503 }) }
